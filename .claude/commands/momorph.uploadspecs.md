...
**CRITICAL RULE:** Before executing any file reading or processing, you MUST verify that the user has provided all 3 required inputs (`input_file_path`, `screenId`, `screen-name`).
...
3) **Resolve Frame ID and Name**
   - **If user chooses an existing frame:**
     - Set `resolved-frame-id` = selected frame's ID.
     - Set `resolved-frame-name` = selected frame's name.
   - **If user chooses to create a new frame:**
     - Call `momorph/create_frame(screenId, screen-name)` to create a new MoMorph Frame.
     - From the tool response, extract and set `resolved-frame-id` and `resolved-frame-name`.

4) **Proceed** to Step 2 with the resolved `resolved-frame-id` and `resolved-frame-name`.
